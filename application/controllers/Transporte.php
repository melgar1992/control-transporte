...
class Transporte extends BaseController
{
    ...
    public function guardarTransporte()
    {
         ...
         if ($this->form_validation->run()) {
             ...
             if (isset($ID_transporte)) {
                 ...
                 $this->guardarDetalleTransporte($ID_transporte, $Distancia, $ID_camion, $ActViaje, $Diesel, $PrecioProveedor, $Precio, $Cantidad, $Comision, $TotalDetalle);
    ...
    private function guardarDetalleTransporte($ID_transporte, $Distancia, $ID_camion, $ActViaje, $Diesel, $PrecioProveedor, $Precio, $Cantidad, $Comision, $Total)
    {
        if (isset($TotalDetalle)) {
            for ($i = 0; $i < count($TotalDetalle); $i++) {
                $camion_actual = $this->Camion_model->obtenerCamionPropio($ID_camion[$i]);
                if (isset($camion_actual)) {
                    $kilometraje = intval($camion_actual['Kilometraje']) + intval($Distancia);
                    $this->Camion_model->actualizarKilometraje($camion_actual['ID_camion'],$kilometraje);
                    $data = array(
