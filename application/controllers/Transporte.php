...
class Transporte extends BaseController
{
    ...
    public function actualizarTransporte()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $transporte_actual = $this->Transporte_model->obtenerTransporte($ID_transporte);
            $this->Transporte_model->editarTransporte($ID_transporte, $Datos_transporte);
            if (isset($ID_transporte)) {

                $this->eliminarDetalleTransporte($ID_transporte, $transporte_actual['Distancia']);
                $this->guardarDetalleTransporte($ID_transporte, $Distancia, $Fecha, $ID_camion, $ActViaje, $Diesel, $PrecioProveedor, $Precio, $Cantidad, $Comision, $Descuento, $TotalDetalle);
                redirect(site_url() . '/Transporte/transporte');
            } else {
                $this->session->set_flashdata('error', 'Ocurrio un error al ingresar los datos del transporte!');
            ...
            for ($i = 0; $i < count($ID_camion); $i++) {
                ...
                if (isset($camion_propio)) {
                    ...
                    $data = array(
                        ...
                        'Descuento' => $Descuento[$i],
                        'Total' => $Total[$i],
                        'ComisionProveedor' => (((floatval($Precio[$i]) - floatval($PrecioProveedor[$i]))) * floatval($Cantidad[$i])) + floatval($Comision[$i]),
    ...
    private function eliminarDetalleTransporte($ID_transporte, $Distancia)
    {
        //Al eliminar los detalles de transporte se tiene que actualizar el kilometraje del camion.
        $detalle_actual = $this->Transporte_model->obtenerDetalleTransporte($ID_transporte);
        if (isset($detalle_actual)) {
            foreach ($detalle_actual as $row) {
                $camion_propio = $this->Camion_model->obtenerCamionPropio($row->ID_camion);
                if (isset($camion_propio)) {
                    $kilometraje = intval($camion_propio['Kilometraje']) - intval($Distancia);
                    $this->Camion_model->actualizarKilometraje($camion_propio['ID_camion'], $kilometraje);
                }
            }
            $this->Transporte_model->eliminarDetalleTransporte($ID_transporte);
        }
    }
    public function eliminarTransporte($ID_transporte)
    {
        ...
        $this->Transporte_model->editarTransporte($ID_transporte, $datos);
        $respuesta = array(
            'tipo' => 'Exitoso',
            'message' => 'Se elimino',
        );
